...
// `forceDark` is for the few surfaces that are dark in EVERY theme — the
// sidebar's nav flyout being the one that exists today (background:
// var(--header-bg), colour: var(--header-text)). Without it, a value tinted
// for a light page lands on dark chrome and the AA guarantee above quietly
// stops holding. It is a per-call override, not a second global mode.
function interpolateRedGreen(ratio, forceDark = false) {
    const t = Math.max(0, Math.min(1, ratio));
    const a = (forceDark || isDarkTheme()) ? DARK_ANCHORS : LIGHT_ANCHORS;
...
export function colorForValue(value, min, max, { forceDark = false } = {}) {
    if (min === max) return interpolateRedGreen(0.5, forceDark);
    const ratio = (value - min) / (max - min);
    return interpolateRedGreen(ratio, forceDark);
}
