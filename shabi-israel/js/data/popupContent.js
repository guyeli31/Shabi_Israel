 ...
 *
 * Consumers:
 *   - the "Explanation and Maths" tool (explanation-and-maths.html) — reads POPUPS to render
...
function prTableSection(lang) {
    return lang === 'he' ? `
                ...
                <p>השורות הן הפרש ה-PR, העמודות הן אורך הדו קרב; כל תא הוא סיכוי הניצחון (%) של השחקן החזק יותר בשילוב הזה.</p>
...
function luckConfidenceExplainerHtml(lang) {
    ...
    if (lang === 'he') {
        return `
            <h4>משמעות Luck</h4>
            <p>האם, בהינתן רמת המשחק של השחקן אל מול רמת יריביו, הוא ניצח <b>יותר</b> ממה שהיה צפוי, או <b>פחות</b>? רשומת ניצחונות־הפסדים גולמית לא יכולה לענות על זה — ניצחון על יריב חזק שווה יותר מניצחון על חלש. המדד הזה משקלל כל תוצאת דו קרב לפי כמה היא הייתה סבירה מלכתחילה, ואז מדווח — כ<b>אחוזון 0–100</b> — עד כמה אנחנו בטוחים שהשחקן בר מזל או חסר מזל. <b>50</b> = בדיוק על פי המודל. מספר הדו קרבות במדגם משפיע על האחוזון: אצל שחקן עם שני ניצחונות שנראים ברי-מזל המדד קרוב הרבה יותר ל-50 מאשר אצל שחקן עם עשרים ניצחונות כאלה — ככל שנאספו יותר דו קרבות, כך אפשר להיות בטוחים יותר.</p>
            ...
            <p>דמיינו "חוגה" נסתרת אחת, <b>θ</b>, שהזיזה את כל דו קרבות השחקן באותו כיוון, והמדידה שלה היא ביחידות log-odds. ערך חיובי (<b>θ &gt; 0</b>) מציין שהשחקן בר מזל, ערך שלילי (<b>θ &lt; 0</b>) — חסר מזל, ואפס (<b>θ = 0</b>) — בדיוק על פי המודל. איננו יודעים את θ — במקום זאת אנחנו מסיקים <b>התפלגות שלמה</b> של הערכים הסבירים עבורה.</p>
            ...
            <p><b>s = ${s}</b> קובע כמה מזל נראה לנו סביר מראש. הוא גם מה שמונע ממדגם זעיר להצהיר בטעות "100% בר מזל": כל עוד לא נאספו הרבה דו קרבות, ה-Prior מושך את המסקנה בחזרה לכיוון 0.</p>
            ...
            <h5>3 · התרומה של כל דו קרב</h5>
            <p>לכל דו קרב לוקחים את סיכוי הניצחון הצפוי <i>p<sub>i</sub></i> (מ-<i>PR Win-Probability Table</i>), הופכים אותו ל-log-odds, ומזיזים אותו ב-θ:</p>
            ...
            <p>כופלים את ה-Prior בתרומה של כל דו קרב:</p>
            <div class="corr-formula">${D_POSTERIOR_MATHML}</div>
            <p>בשונה מה-Prior, העקומה הזו <b>אינה</b> מנורמלת מחדש — השטח שמתחתיה מתכווץ עם כל דו קרב שמכפיל אותה, ולכן יותר דו קרבות נותנים תוצאה חדה ובטוחה יותר. אבל למדד עצמו הנרמול לא משנה: הוא <b>יחס שטחים</b>, וקבוע הכפל מצטמצם.</p>
            ...
            <p>ה-Prior תמיד משאיר משקל הסתברותי לשני הצדדים, ולכן אפילו שחקן שניצח את <b>כל</b> הדו קרבות שלו לעולם לא יגיע ל-100: ככל שהרשומה שלו נשענת על פחות דו קרבות, כך המדד נמשך חזק יותר בחזרה לעבר 50. משום כך אין צורך באזהרת "מדגם קטן" נפרדת — גודל המדגם כבר בא לידי ביטוי במספר עצמו.</p>
            ...
            <p>שימו לב שאף אחד מהם אינו מגיע ל-0 או ל-100: עם שני דו קרבות בלבד, ה-Prior עדיין מושך את שתי התוצאות אל עבר האמצע.</p>
...
export const POPUPS = [
    // ============ 1 · Championship Predictor (dashboard) ============
    {
        ...
        render(lang) {
            return lang === 'he' ? `
                <h3>איך זה עובד</h3>
                <p>מנוע החיזוי משחק את כל הדו קרבות שנותרו בליגה <b>אלפי פעמים</b>, כל פעם עם תוצאות מעט שונות, וסופר כמה פעמים כל שחקן מסיים ראשון. שחקן שזוכה באליפות ב-6 מתוך כל 10 עונות מדומות, למשל, מקבל סיכוי של <b>60%</b>. ככל שנותרו יותר דו קרבות, כך המרוץ פתוח יותר.</p>
                <h4>כמה חזק כל שחקן?</h4>
                <p>העוצמה של שחקן נגזרת מה-<b>PR</b> העדכני שלו (נמוך יותר = טוב יותר). אבל אף אחד לא משחק באותה רמה בכל דו קרב, אז בכל דו קרב מדומה כל שחקן משחק ברמה קצת מעל או מתחת לרמתו הרגילה. השונות הזו מאפשרת הפתעות — למועדף יכול להיות דו קרב חלש, והפוך.</p>
                <h4>מי מנצח בדו קרב בודד?</h4>
    ...
    // ============ 2 · What If (dashboard) ============
    {
        ...
        render(lang) {
            return lang === 'he' ? `
                <h4>מה אם</h4>
                <p><b>למה זה כאן:</b> מרוץ האליפות תלוי בתוצאות שעוד לא נקבעו. טבעי לשאול "מה היה קורה אילו הדו קרב הזה היה נגמר אחרת?" — הכלי הזה נותן לכם לענות על השאלה הזו בעצמכם: לקבע תוצאות, ולראות מיד איך סיכויי האליפות זזים.</p>
                <p>בחרו כל דו קרב מתוזמן בליגה וכפו עליו תוצאה (A מנצח, B מנצח, או לא שוחק). ניתן להוסיף כמה דו קרבות שרוצים, ואז ללחוץ <b>הרץ סימולציה</b> כדי לראות איך סיכויי האליפות היו משתנים בתרחיש החלופי הזה.</p>
                <ul>
    ...
    // ============ 3 · Player PR ↔ Result Correlation (dashboard) ============
    {
        ...
        render(lang) {
            return lang === 'he' ? `
                ...
                <h4>רקע</h4>
                <p>לכל דו קרב יש סיכוי ניצחון צפוי לשחקן החזק יותר, הנקבע על ידי שני משתנים: <b>הפרש ה-PR</b> בין שני השחקנים, ואורך הדו קרב. זהו מודל קבוע ומפורסם (שאינו מותאם לתוצאות הליגה הזו עצמה) — אותה טבלה משמשת לדירוג כל דו קרב בעמוד הזה, ללא תלות במי שיחק אותו או מתי.</p>
                ${prTableSection('he')}
                <h4>איך קוראים את הגרף</h4>
                <p>לכל שחקן קיים גרף משלו, עם נקודה אחת על הגרף עבור כל דו קרב ששיחק, הממוקמת לפי <b>יתרון ה-PR</b> שלו באותו דו קרב — ה-PR של היריב פחות שלו (PR נמוך יותר = פחות טעויות, כך שיתרון חיובי משמעותו שהשחקן עצמו שיחק את הדו קרב ברמה טובה יותר מיריבו). נקודה שנמצאת יותר <b>ימינה</b> אומרת שהשחקן שיחק טוב מהיריב באותו דו קרב; יותר <b>שמאלה</b> אומרת שהיריב שיחק טוב ממנו. <span style="color:var(--color-win)">ירוק</span> = ניצחון, <span style="color:var(--color-loss)">אדום</span> = הפסד.</p>
                <p>בגרף הדוגמה שלמטה יש לשחקן שלושה דו קרבות: <span style="color:var(--color-win)">ניצחון</span> ביתרון <span dir="ltr">+5</span> (שיחק הרבה יותר טוב — ניצחון צפוי), <span style="color:var(--color-win)">ניצחון</span> ביתרון <span dir="ltr">&minus;3</span> (שיחק פחות טוב ובכל זאת ניצח — תוצאה מפתיעה שנזקפת למזל טוב), ו<span style="color:var(--color-loss)">הפסד</span> ביתרון <span dir="ltr">+2</span> (היה מעט עדיף אך הפסיד — חוסר מזל קל). <b>מעל</b> הגרף מופיע מדד ה-<b>Luck</b> בדיוק כפי שהוא מוצג בדשבורד — הוא מסכם את שלוש הנקודות למספר יחיד בסקאלה 0–100. כאן ההפתעה החיובית גוברת על ההפסד הקל, ולכן ה-Luck יוצא <b>63</b> (בר מזל במעט). המשמעות המלאה של המדד מוסברת בהמשך.</p>
    ...
    // ============ 4 · League PR ↔ Result Correlation (dashboard) ============
    {
        ...
        render(lang) {
            return lang === 'he' ? `
                <p><b>למה זה כאן:</b> במבט על כלל הליגה — האם השחקן הטוב יותר באמת מנצח לעתים קרובות יותר, על פני מאות דו קרבות? כאן בודקים את זה במצטבר.</p>
                <h4>רקע</h4>
                <p>לכל דו קרב יש סיכוי ניצחון צפוי לשחקן החזק יותר, הנקבע לפי <b>הפרש ה-PR</b> בין שני השחקנים ואורך הדו קרב &mdash; מודל קבוע ומפורסם, שאינו מותאם לתוצאות הליגה הזו עצמה. החלק הזה בוחן את המודל הזה באופן מצרפי, על פני הרבה דו קרבות בבת אחת, ולא שחקן אחד בכל פעם.</p>
                ${prTableSection('he')}
                <h4>החישוב</h4>
                ...
                <h4>הבקרות</h4>
                <ul>
                    <li><b>הזזת פער PR</b> (בשורת "כל הליגות" בלבד) מוסיפה קבוע ליתרון של כל דו קרב לפני שההיסטוגרמה, התאמת הגאוס וטבלת אימות המודל למטה מחושבות מחדש &mdash; דרך לבדוק האם נקודת הכיול של המודל מוזזת בכמות קבועה.</li>
                    <li><b>חתוך ל-99%</b> מקרב את התצוגה של השורה לאמצע 99% מהדו קרבות שלה, ומסתיר את העמודות החריגות &mdash; לתצוגה בלבד, הנתונים עצמם אינם מושפעים.</li>
                    <li><b>התאמת גאוס</b> מציגה עקומה נורמלית שמותאמת לממוצע ולסטיית התקן של אותה שורה (מוצגים כ-&mu; ו-&sigma;), בתוספת קווים מקווקווים בממוצע ובמרחק &plusmn;1 סטיית תקן &mdash; הפניה חזותית בלבד, לא טענה שערכי היתרון אכן מתפלגים נורמלית. מנוטרל כשלשורה אין עדיין מספיק דו קרבות שממוצע/סטיית תקן יהיו משמעותיים.</li>
    ...
    // ============ 5 · Luck Percentile (landing page) ============
    {
        ...
        render(lang) {
            return lang === 'he' ? `
                <p>לכל שחקן ברשימה מוצג <b>אחוזון מזל</b> יחיד, 0–100, המסכם את כל הדו קרבות המדורגים שלו בכל הליגות: עד כמה אנחנו בטוחים שהוא בר מזל (מעל המודל) או חסר מזל (מתחתיו), אחרי ששוקללה חוזק היריב בכל דו קרב. להלן החישוב המלא.</p>
                ${prTableSection('he')}
                ${luckConfidenceExplainerHtml('he')}
            ` : `
    ...
    // ============ 5b · Total PR ↔ Result (player page) ============
    {
        ...
        render(lang) {
            return lang === 'he' ? `
                ...
                <h4>רקע</h4>
                <p>PR (Performance Rating) מודד את איכות המהלכים בדו קרב &mdash; <b>נמוך יותר = טוב יותר</b>. לכל דו קרב יש סיכוי ניצחון צפוי לשחקן החזק יותר, הנקבע לפי הפרש ה-PR בין השניים ואורך הדו קרב. זהו מודל קבוע ומפורסם, שאינו מותאם לתוצאות הליגות שלנו.</p>
                ${prTableSection('he')}
                <h4>החישוב</h4>
                <p>לכל דו קרב ששוחק מחושב <b>הפרש PR</b> מנקודת מבטו של השחקן:</p>
                <p style="text-align:center"><i>הפרש</i> = <i>PR</i><sub>יריב</sub> &minus; <i>PR</i><sub>שחקן</sub></p>
                <p>הפרש <b>חיובי</b> אומר שהשחקן שיחק את הדו קרב ברמה טובה יותר מיריבו; הפרש <b>שלילי</b> אומר שהיריב שיחק טוב ממנו. הדו קרבות מקובצים ל-bins ברוחב נקודת PR אחת, וגובה כל עמודה הוא חלקם (%) מכלל הדו קרבות של סוג הליגה הנבחר.</p>
                <p>דו קרבות בעלי תוצאה טכנית, וכן דו קרבות ללא רישום PR, אינם נכללים.</p>
                <h4>איך קוראים את הגרף</h4>
                ...
                <ul>
                    ...
                    <li><b>All</b> &mdash; כל הדו קרבות יחד, ללא קשר לתוצאה: זו התפלגות הפרשי ה-PR כשלעצמה.</li>
                </ul>
                ...
                <h4>דוגמה</h4>
                <p>נניח ששחקן ניצח דו קרב שבו ה-PR שלו היה 4.5 וה-PR של יריבו 7.0. ההפרש הוא <span dir="ltr">7.0 &minus; 4.5 = +2.5</span>, ולכן הדו קרב נופל ב-bin <span dir="ltr">[2, 3)</span> של סדרת ה-<b>Wins</b>. אותו שחקן הפסיד דו קרב שבו שיחק 5.0 מול 8.0 של היריב: ההפרש הוא <span dir="ltr">+3.0</span> &mdash; הוא שיחק טוב יותר ובכל זאת הפסיד &mdash; ולכן הדו קרב נופל ב-bin <span dir="ltr">[3, 4)</span> של סדרת ה-<b>Losses</b>. אצל שחקן שתוצאות הדו קרבות שלו תואמות לאיכות המשחק, עקומת ה-Wins יושבת ימינה מעקומת ה-Losses; ככל ששתי העקומות חופפות יותר, כך התוצאות היו פחות תלויות בהפרש ה-PR.</p>
                ...
                <h4>הפס שמעל כל גרף: Luck</h4>
                <p>מעל כל גרף יושב פס יחיד: המילה <b>Luck</b>, סקאלת צבע עם סמן ומספר, והמסקנה במילים (למשל &rlm;<b>Slightly lucky</b>). הגרף מראה <i>כמה טוב</i> השחקן שיחק בכל דו קרב; הפס מסכם ב<b>מספר אחד</b> את השאלה הנפרדת &mdash; האם התוצאות שיצאו לו היו טובות יותר או פחות ממה שרמת המשחק שלו הצדיקה.</p>
                <p>הפס מתאר בדיוק את הדו קרבות המצוירים מתחתיו: הוא מחושב מחדש כשמחליפים סוג ליגה או אורך דו קרב, ולכל גרף בערימה יש פס משלו. אותם דו קרבות בדיוק שנספרים בכותרת (<span dir="ltr">x/y matches</span>) הם שנכנסים לחישוב. זהו אותו מדד ממש המוצג בכרטיס <b>Luck Percentile</b> בעמוד הבית ובסקשן <b>Total Luck</b>, ובאותה סקאלה &mdash; ההבדל הוא רק אילו דו קרבות נכללים.</p>
                ${luckConfidenceExplainerHtml('he')}
            ` : `
                ...
                <h4>The bar above each chart: Luck</h4>
    ...
    // (buildExplanationTableHtml). Only the prose is stored here.
    {
        ...
        render(lang) {
            return lang === 'he' ? `
                ...
                <p><b>למה זה כאן:</b> ה-<i>PR Win-Probability Table</i> היא טבלה קבועה ומפורסמת שנותנת, לכל פער PR ואורך דו קרב, את הסיכוי שהמועדף ינצח — וכל העמוד הזה נשען עליה. אבל האם הטבלה הזו באמת נכונה עבור השחקנים שלנו? הבדיקה הזו מרכזת את כל דו קרבות ${leagueTypePill('doubling')} ששוחקו אי פעם (באותו אורך דו קרב) ושואלת, עבור כל פער PR, האם המועדף ניצח בתדירות שהטבלה חוזה.</p>
