...
/** Must match adminPage.js's DEFAULT_VIEW — the view whose URL is bare. */
const ADMIN_DEFAULT_VIEW = 'leagues';

const NAV_ITEMS = [
    { key: 'dashboard', label: 'Main Dashboard', icon: '🏠', href: 'index.html?edit' },
    ...
    { key: 'pending-changes', label: 'Pending Changes', icon: '📝', badge: true },
    { key: 'historical-changes', label: 'Historical Changes', icon: '🕘' },
...
export function buildAdminSidebarHtml(opts = {}) {
    ...
    const navHtml = NAV_ITEMS.map(item => {
        ...
        if (mode === 'link') {
            // The default view carries no hash — see adminPage.js's DEFAULT_VIEW
            // and CLAUDE.md § URL contract. Linking to `admin.html#leagues` would
            // hand the default view a second URL.
            const href = item.href
                || (item.key === ADMIN_DEFAULT_VIEW ? 'admin.html' : `admin.html#${item.key}`);
