...
/**
 * "Automatic match update" = a match result written by machinery rather than by
 * a person: the scheduled sync, a workflow run, or an e-mail report. All three
 * land as an automation-authored batch whose change is a match one, so one
 * predicate covers them and no new column is needed.
 *
 * Deliberately narrow. Automation also authors overrides and one-off backfills,
 * and those are decisions worth reading — only the routine per-match traffic is
 * noise, and only that is hidden.
 */
function isAutoMatchUpdate(b) {
    return (b.changed_by || '').includes('automation') && /^match/i.test(b.specific || '');
}

/**
 * Exclude toggle. Off on every load — the full history is the truth, and a
 * filter that silently persists would let an admin conclude a sync never ran.
 *
 * Filtering is client-side ON TOP of the fetched page, so "Show last 100" keeps
 * meaning "the last 100 changes recorded" rather than quietly becoming "however
 * many of the last 100 survived a filter". That does mean the visible count
 * drops, which is why the note spells out exactly how many rows are hidden
 * instead of leaving the admin to wonder where they went.
 */
function bindAutoFilter(container) {
    const slot = container.querySelector('#history-filter-slot');
    const note = container.querySelector('#history-filter-note');
    if (!slot) return;

    mountFilterToggle(slot, {
        id: 'history-hide-auto',
        label: 'Hide automatic match updates',
        title: 'Match results written by the scheduled sync, a workflow run, or an e-mail report',
        pressed: false,
        onToggle: (on) => {
            const rows = container.querySelectorAll('.history-item[data-auto-match="1"]');
            rows.forEach((li) => { li.hidden = on; });
            if (!note) return;
            note.hidden = !on || rows.length === 0;
            note.textContent = rows.length === 1
                ? '1 automatic match update hidden'
                : `${rows.length} automatic match updates hidden`;
        },
    });
}
...
export async function renderHistoricalChanges(container) {
    ...
    const limitBarHtml = `
        ...
        <div class="history-filter-bar">
            <span id="history-filter-slot"></span>
            <span class="history-filter-note" id="history-filter-note" hidden></span>
        </div>`;
    ...
    const itemsHtml = data.map((b) => {
        ...
        return `
            <li class="pending-item history-item"${isAutoMatchUpdate(b) ? ' data-auto-match="1"' : ''} data-batch-id="${b.id}">
