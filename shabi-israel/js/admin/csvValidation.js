 ...
 * @param {object} [opts]
 * @param {boolean} [opts.emptyBaseline] — compare against a CLEAN league instead
 *   of reading any current state. Used by Add New League, where the league does
 *   not exist yet: every played match in the file is new by definition, and the
 *   admin wants to see exactly that list. Also keeps the report from picking up
 *   a same-named staged league, or the form's own previous import.
 * @returns {Promise<object>} structured report (see fields below)
 */
export async function computeCsvImportReport(leagueId, newCsvText, opts = {}) {
    const { csv: curCsv, overrides } = opts.emptyBaseline
        ? { csv: '', overrides: [] }
        : await readCurrentState(leagueId);
 ...
 * @param {string} [okLabel] — wording for the clean-report state (Add New League
 *   composes into the pending league instead of staging, so it says so).
 */
