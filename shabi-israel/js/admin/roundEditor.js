...
function populatePlayerDatalist(root, playerSet, customFlags = {}) {
    ...
    // The metadata itself is kept too: it is what `displayPlayerName` and
    // `alternateName` read, so the two names below stay correct once it arrives.
    const titleMap = {};
    let metaMap = {};
    import('../data/supabasePlayersMetadata.js')
        ...
        .then(meta => {
            metaMap = meta || {};
            for (const [n, m] of Object.entries(metaMap)) { const t = getTitleAbbreviationsHtml(m); if (t) titleMap[n] = t; }
        })
        .catch(() => {});
    mountCombobox(input, {
        // Sorted on every open, not once at mount: the metadata that decides
        // which name is SHOWN arrives asynchronously, and a list ordered by the
        // username while displaying full names reads as unsorted.
        getOptions: () => [...names].sort((a, b) =>
            displayPlayerName(a, metaMap[a]).localeCompare(displayPlayerName(b, metaMap[b]))),
        labelFor: (p) => displayPlayerName(p, metaMap[p]),
        altFor:   (p) => alternateName(p, metaMap[p]),
        decorate: (p) => ({ flagCode: getFlagCode(p, customFlags), titleHtml: titleMap[p] || '' }),
        // No `identity` and no `browseOnOpen`, and both omissions are the canon
        // rather than gaps: this is a FILTER box — its text is a query that
        // narrows the round cards below, not a player who has been chosen — so
        // there is no subject for the field to keep showing, and emptying it on
        // re-focus would throw away the filter the admin is in the middle of.
    });
}
