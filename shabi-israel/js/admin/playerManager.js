...
import { BMAB_TITLES, bmabSelectOptionsHtml, COUNTRIES, getChampionshipTooltip, getTitleAbbreviationsHtml } from '../data/titleConstants.js';
...
/** Order the player list by the name it SHOWS, so the sort still reads as A→Z
 *  when "Show name as" is set to full names. Every place that (re)builds
 *  `_state.players` uses it, or a rename/add drops a row in the wrong place. */
const byDisplayName = (a, b) =>
    displayPlayerName(a, _state.metadata[a]).localeCompare(displayPlayerName(b, _state.metadata[b]));

let refreshBadgeFn = null;
let _state = {
    metadata: {},
    players: [], // sorted by display name (active + inactive pre-registered)
...
export async function renderPlayerAdmin(container, refreshBadge) {
    ...
    try {
        ...
        // `metadata` is read directly rather than through `byDisplayName` here
        // because `_state.metadata` is not assigned until the next line.
        _state.players = [...set].sort((a, b) =>
            displayPlayerName(a, metadata[a]).localeCompare(displayPlayerName(b, metadata[b])));
...
function renderShell(container) {
    container.innerHTML = `
        ...
        <div class="admin-card">
            <div class="form-group">
                ...
                <input type="text" id="player-search" class="app-search-input"
    ...
    // mobile search sheet, so on a phone it was the one player search you had to
    // type into a 14px field. It now runs on the project's ONE canonical search
    // field like every other player picker. The admin-only state tags survive as
    // the row's badge, and the picked player stays in the field with their flag
    // and titles, so the form below always has a visible subject.
    mountCombobox(input, {
        getOptions: () => _state.players,
        // Both names, and both searchable — the canonical pair. This used to
        // hard-code `sublabel: meta.fullName`, which showed the full name but
        // matched only the username, so an admin who knew a player by their real
        // name could read it on screen and still get no results typing it.
        labelFor: (name) => displayPlayerName(name, _state.metadata[name]),
        altFor:   (name) => alternateName(name, _state.metadata[name]),
        decorate: (name) => {
            ...
            const badge = isInactive ? { text: 'inactive', kind: 'unplayed' }
                : isHidden ? { text: 'hidden', kind: 'unplayed' }
        ...
        identity: true,
        allowFreeText: false,          // there is nothing to edit but an existing player
        browseOnOpen: true,            // the value is a standing selection, not a query
        onPick: (name) => {
            // The field shows the DISPLAY name (the identity chrome beside it is
            // that player's flag and titles); `selectPlayer` still receives the
            // username key, which is what every record is filed under.
            input.value = displayPlayerName(name, _state.metadata[name]);
            selectPlayer(container, name);
        },
    });
    ...
    host.innerHTML = `
        ...
        <div class="admin-card">
            ...
            <iframe class="player-preview-frame"
                    src="player.html?player=${encodeURIComponent(name)}&preview"
...
function saveNewPlayer(container, host, form) {
    ...
    _state.players = [...new Set([..._state.players, nickname])].sort(byDisplayName);
