...
export function buildMatchupPreset({ rows, playerName, opponent, enrich = {} }) {
    const cols = [
        ...
        // The winner is a PLAYER, so it is rendered as one: flag · name · title
        // badges, the same identity chip every other player reference on the
        // site carries. `enrich.playerIdentity` supplies it — the preset itself
        // has no access to flags or metadata, exactly as with the league link.
        // Falls back to the bare display name when a caller does not pass one.
