...
/**
 * Turn an export's base name into a filename that is safe on every OS.
 *
 * Export filenames deliberately carry the SAME text the image shows —
 * league title, league type, and the player name exactly as the "Show name
 * as" setting renders it — so a downloaded PNG is identifiable in a folder
 * without opening it. That text is free-form admin input, which is why it
 * cannot go into `a.download` raw: a full name like "Avi / Danny" or a title
 * with ":" contains characters Windows forbids in a filename, and the browser
 * silently mangles or drops the download instead of erroring.
 *
 * So every download goes through here: whitespace runs collapse to a single
 * "_" (a full name with spaces becomes one underscored token), the characters
 * Windows/macOS reserve (\ / : * ? " < > |) plus control characters become
 * "_", and leading/trailing dots and underscores are trimmed off (a name
 * starting with "." would create a hidden file). Runs of underscores collapse
 * so a stripped character never leaves "__" behind.
 */
