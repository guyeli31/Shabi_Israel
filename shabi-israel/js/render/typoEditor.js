...
import { spliceQueryParam } from '../utils/queryString.js';
...
const GROUPS = [
    ...
    // ── Tables (per page) ──
    // A1-A7 — index.
    // A1/A2 are t6, not t5: both render `.font-small` (= --fs-085 = t6) in
    // css/index-dashboard.css, and every other index table matches them. The
    // t5 they used to carry described no table on the page — applying the
    // editor's default would have silently enlarged the two of them.
    { id: 'a1-table', pages: ['index'], tier: 't6', bold: false, label: 'A1 — Completed Leagues', sel: '[data-mf-table-id="A1"] th, [data-mf-table-id="A1"] td', isTable: true, tableSel: '[data-mf-table-id="A1"]' },
    { id: 'a2-table', pages: ['index'], tier: 't6', bold: false, label: 'A2 — Annual Leaderboard', sel: '[data-mf-table-id="A2"] th, [data-mf-table-id="A2"] td', isTable: true, tableSel: '[data-mf-table-id="A2"]' },
    ...
    { id: 'a7-table', pages: ['index'], tier: 't6', bold: false, label: 'A7 — Players directory',  sel: '[data-mf-table-id="A7"] th, [data-mf-table-id="A7"] td', isTable: true, tableSel: '[data-mf-table-id="A7"]' },
...
function syncOverlayFlag() {
    ...
    const has = new URLSearchParams(loc.search).get('searchoverlay') === 'force';
    if (want === has) return false;               // already in sync — no reload
    // Splice the one flag instead of re-serialising the whole query: the preview
    // URLs carry `?league=Shabi%20Israel%20…`, and URLSearchParams.toString()
    // would rewrite those spaces as `+`, so the iframe would show a different
    // URL than the site itself. See js/utils/queryString.js.
    iframeEl.src = spliceQueryParam('searchoverlay', want ? 'force' : null, loc);
