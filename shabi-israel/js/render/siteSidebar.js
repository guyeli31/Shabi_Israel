 ...
 *  Leaders       — links to index.html?tab=leaders#leaders-panel (jumps straight to the leaderboard panel, tab strip scrolled out of view).
 *  Settings ▸
...
export function mountSiteSidebar(opts = {}) {
    ...
    // Everything currently in <body> moves inside .site-main — EXCEPT the
    // loading screen. Moving a node re-inserts it, and re-insertion restarts
    // every CSS animation on it: the splash's logo and ring would visibly
    // fade and scale in a second time, mid-load, which reads as the whole
    // loading screen restarting. It is also position:fixed and belongs to no
    // page region, so it has no business inside the layout in the first place.
    const existing = [...document.body.childNodes]
        .filter(n => n.id !== 'logo-splash');
...
function renderShell(sidebar, opts) {
    ...
    sidebar.innerHTML = `
        ...
        <nav aria-label="Site sections" class="site-nav-tree">
            ...
            <a class="site-nav-item" href="index.html?tab=leaders#leaders-panel" data-view="leaders">
            ...
            async suggest(query) {
                ...
                for (const l of leagues) {
                    items.push({
                        ...
                        href: preview ? `${leagueUrl(l.id)}&preview` : leagueUrl(l.id),
                    });
                }
                for (const p of players) {
                    ...
                    items.push({
                        ...
                        href: preview ? `${playerUrl(p.name)}&preview` : playerUrl(p.name),
                    });
                }
                return items;
            },
