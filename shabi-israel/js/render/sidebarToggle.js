...
export function mountSidebarToggle(opts = {}) {
    ...
    // Analytics: the hamburger lives on <body> (outside <aside>), so the click
    // listener's "aside a, aside button" menu branch never saw it and the nav
    // toggle went untracked. data-track opts it in generically → logged as
    // "Menu: Navigation" and shown with the bare ☰ (it IS the menu), the one
    // click that owns that glyph — every menu ITEM shows ☰ + its own icon.
    hamburger.setAttribute('data-track', 'Menu: Navigation');
