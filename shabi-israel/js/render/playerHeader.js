...
export function buildHeaderTitles(meta) {
    ...
    if (champ) {
        out.push({
            label: champ.label,
            abbreviation: champ.abbreviation, // WC / NC — stable slug for analytics
            icon: CHAMP_ICON,
            tier: champ.tier,
            kind: 'champ',
    ...
    if (meta?.bmabTitle) {
        ...
        if (info) {
            out.push({
                label: info.label,
                abbreviation: info.abbreviation, // e.g. G2
...
export function renderV7Header(target, data) {
    ...
    const chipsHtml = (data.titles || []).map(t =>
        `<span class="pg-v7-chip pg-v7-tier-${t.tier}" data-tooltip-tap tabindex="0" data-track="Title: ${escapeHtml(t.abbreviation || t.label)}" title="${escapeHtml(t.tooltip)}">` +
...
export function renderV12Header(target, data) {
    ...
    const ribbons = (data.titles || []).map(t =>
        `<span class="pg-v12-titleribbon pg-tier-${t.tier}" data-tooltip-tap tabindex="0" data-track="Title: ${escapeHtml(t.abbreviation || t.label)}" title="${escapeHtml(t.tooltip)}">` +
