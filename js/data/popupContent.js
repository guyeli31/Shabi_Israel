...
function luckConfidenceExplainerHtml(lang) {
    ...
    if (lang === 'he') {
        return `
            ...
            <p>דמיינו "חוגה" נסתרת אחת, <b>θ</b>, שהזיזה את כל משחקי השחקן באותו כיוון, נמדדת ב-log-odds. <b>θ &gt; 0</b> = השחקן בר מזל, <b>θ &lt; 0</b> = חסר מזל, <b>θ = 0</b> = בדיוק על פי המודל. איננו יודעים את θ — במקום זאת אנחנו מסיקים <i>התפלגות שלמה</i> של ערכים סבירים עבורו.</p>

            <h5>2 · נקודת הפתיחה: ה-Prior</h5>
            <p>לפני שמסתכלים על תוצאה כלשהי, אנחנו מניחים ש-θ קרוב לוודאי קטן: פעמון שמרוכז סביב 0 ברוחב <i>s</i>.</p>
            <div class="corr-formula">${D_PRIOR_MATHML}</div>
            <p><b>s = ${s}</b> קובע כמה מזל נראה לנו סביר מראש. הוא גם מה שמונע ממדגם זעיר לצרוח "100% בר מזל": כל עוד לא נאספו הרבה משחקים, ה-Prior מושך את המסקנה בחזרה לכיוון 0.</p>
            <div class="contrib-grid">${gPrior}</div>
            <p>הגרף מראה את ה-Prior לשלושה ערכי <i>s</i>: קטן יותר (<span style="color:#a855f7">סגול</span>) = פעמון צר וגבוה, המניח שהמזל כמעט תמיד קטן; גדול יותר (<span style="color:#14b8a6">טורקיז</span>) = פעמון רחב ושטוח, שמאפשר למזל לזוז רחוק. הקו המלא הוא ברירת המחדל <b>s = 1</b>. שטח כל עקומה הוא 1 (זו התפלגות הסתברות).</p>
            ...
            <p>ואז שואלים כמה θ נתון <i>מסביר</i> את התוצאה בפועל:</p>
            ...
            <p>שימו לב לגרף השלישי: כשהשחקן היה כבר מועדף של 85%, ניצחון בקושי מזיז את העקומה — התוצאה הייתה <i>צפויה</i>, ולכן היא נושאת מעט מאוד מידע על מזל. לעומת זאת ניצחון של נחות (או הפסד של מועדף) מטה את העקומה בחדות. העקמומיות הזו — כמה כל תוצאה מפתיעה — היא כל העיקר.</p>

            <h5>4 · צירוף הכול: ה-Posterior</h5>
            <p>כופלים את ה-Prior בתרומה של כל משחק:</p>
            <div class="corr-formula">${D_POSTERIOR_MATHML}</div>
            <p>בשונה מה-Prior, העקומה הזו <b>אינה</b> מנורמלת מחדש — השטח שמתחתיה מתכווץ עם כל משחק שמכפיל אותה, ולכן יותר משחקים נותנים פסק־דין חד ובטוח יותר. אבל למדד עצמו הנרמול לא משנה: הוא <i>יחס שטחים</i>, וקבוע הכפל מצטמצם.</p>
            ...
            <p>מכיוון שה-Prior תמיד שומר משקל בשני הצדדים, אפילו רשומת "כל הניצחונות" נוחתת מתחת ל-100, נמשכת לכיוון 50 בהתאם לכמה מעט משחקים היא נשענת עליהם. כך אין צורך בסף "מדגם קטן" נפרד — קוטן המדגם כבר מקופל במספר.</p>
...
export const POPUPS = [
    // ============ 1 · Championship Predictor (dashboard) ============
    {
        ...
        render(lang) {
            return lang === 'he' ? `
                ...
                <ul>
                    <li><b>ליגות הכפלות ו-UBC:</b> השחקן עם ה-<b>PR הממוצע</b> הטוב יותר (נמוך יותר) לאורך העונה מדורג גבוה יותר.</li>
    ...
    // ============ 3 · Player PR ↔ Result Correlation (dashboard) ============
    {
        ...
        render(lang) {
            return lang === 'he' ? `
                <p><b>למה זה כאן:</b> האם שחקן ניצח כי שיחק טוב יותר, או כי הקוביות האירו לו פנים? הסקשן הזה מיישר, לכל שחקן, עד כמה טוב הוא שיחק כל משחק (לפי PR) מול השאלה אם ניצח אותו — ומזקק מכך ציון מזל יחיד.</p>
                <h4>רקע</h4>
                ...
                <p>בגרף הדוגמה שלמטה יש לשחקן שלושה משחקים: <span style="color:var(--color-win)">ניצחון</span> ביתרון <span dir="ltr">+5</span> (שיחק הרבה יותר טוב — ניצחון צפוי), <span style="color:var(--color-win)">ניצחון</span> ביתרון <span dir="ltr">&minus;3</span> (שוחק פחות טוב ובכל זאת ניצח — הפתעה, מזל טוב), ו<span style="color:var(--color-loss)">הפסד</span> ביתרון <span dir="ltr">+2</span> (היה מעט עדיף אך הפסיד — חוסר מזל קל). <b>מעל</b> הגרף מופיע מדד ה-<b>Luck</b> בדיוק כפי שהוא מוצג בדשבורד — הוא מסכם את שלוש הנקודות למספר יחיד בסקאלה 0–100. כאן ההפתעה החיובית גוברת על ההפסד הקל, ולכן ה-Luck יוצא <b>63</b> (בר מזל במעט). המשמעות המלאה של המדד מוסברת בהמשך.</p>
    ...
    // ============ 4 · League PR ↔ Result Correlation (dashboard) ============
    {
        ...
        render(lang) {
            return lang === 'he' ? `
                ...
                <h4>רקע</h4>
                <p>לכל משחק יש סיכוי ניצחון צפוי לשחקן החזק יותר, הנקבע לפי <b>הפרש ה-PR</b> בין שני השחקנים ואורך המשחק &mdash; מודל קבוע ומפורסם, שאינו מותאם לתוצאות הליגה הזו עצמה. הסקשן הזה בוחן את המודל הזה באופן מצרפי, על פני הרבה משחקים בבת אחת, ולא שחקן אחד בכל פעם.</p>
                ${prTableSection('he')}
                <h4>החישוב</h4>
                ...
                <p><b>הליגה &mdash; כל המשחקים</b> מרכזת את כל המשחקים ששוחקו בליגה הזו. <b>כל משחקי הליגות</b> מרכזת את כל המשחקים ששוחקו אי פעם בכל הליגות מ<i>אותו סוג ליגה ואותו אורך משחק</i> (פער PR נתון משמעותי יותר במשחק ארוך יותר, כך שערבוב אורכי משחק שונים היה מטשטש את ההשוואה). למשחקים עם תוצאה טכנית אין PR אמיתי, והם אינם נכללים באף אחת מהשורות.</p>
                <h4>איך קוראים את הגרף</h4>
                <p>שתי השורות חולקות ציר אחד, בגודל שמתאים ליתרון הרחב ביותר שנצפה בליגה (או בהיסטוריה של כל הליגות, לאחר שהיא נטענת) &mdash; בלתי תלוי בציר של שורות השחקנים שלמעלה בעמוד.</p>
                <h4>הבקרות</h4>
                <ul>
            ...
            ` : `
                ...
                <h4>Reading the graph</h4>
                <p>Both rows share one axis, sized to the widest advantage seen in the league (or across all-time history, once that loads) &mdash; independent of the player rows' axis further up the page.</p>
                <h4>The controls</h4>
                <ul>
    ...
    // ============ 5b · Total PR ↔ Result (player page) ============
    {
        ...
        render(lang) {
            return lang === 'he' ? `
                <p><b>למה זה כאן:</b> כמה טוב השחקן צריך לשחק כדי לנצח? הסקשן הזה לוקח את כל משחקיו המדורגים, בכל הליגות, ושואל איך <b>הפרש ה-PR</b> באותו משחק התחלק בין המשחקים שניצח לבין אלה שהפסיד.</p>
                <h4>רקע</h4>
                ...
                <ul>
                    ...
                    <li>ה-<b>PILLS</b> שלמעלה מצמצמים את המשחקים לסוג ליגה אחד (Doubling / UBC), או מאחדים את כולם ב-<b>All</b>. ליגות רגילות אינן נכללות כלל &mdash; הן אינן רושמות PR.</li>
            ...
            ` : `
                ...
                <ul>
                    ...
                    <li>The <b>pills</b> above narrow the matches to one league type (Doubling / UBC), or pool them all under <b>All</b>. Regular leagues never appear &mdash; they record no PR.</li>
    ...
    // (buildExplanationTableHtml). Only the prose is stored here.
    {
        ...
        render(lang) {
            return lang === 'he' ? `
                ...
                <p><b>למה זה כאן:</b> ה<i>PR Win-Probability Table</i> היא טבלה קבועה ומפורסמת שנותנת, לכל פער PR ואורך משחק, את הסיכוי שהמועדף ינצח — וכל העמוד הזה נשען עליה. אבל האם הטבלה הזו באמת נכונה עבור השחקנים שלנו? הבדיקה הזו מרכזת את <b>כל משחקי ההכפלות</b> ששוחקו אי פעם (באותו אורך משחק) ושואלת, עבור כל פער PR, האם המועדף ניצח בתדירות שהטבלה חוזה.</p>
            ...
            ` : `
                ...
                <p><b>Why it's here:</b> the <i>PR Win-Probability Table</i> is a fixed, published table giving, for every PR gap and match length, the favourite's win chance — and this whole page leans on it. But is that table actually right for our players? This check pools <b>every doubling-cube match</b> ever played (at this match length) and asks, for every PR gap, whether the favourite won as often as the table predicts.</p>
