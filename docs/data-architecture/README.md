...
6. **[`05-projection-cache.md`](05-projection-cache.md)** — **implemented on local Docker; not on cloud.** Move the Title Race chart's Monte Carlo projections out of the visitor's browser (measured: 22 s of frozen CPU per visit) and into a derived table written by a GitHub Action. Covers the table, the triggers, the invalidation rule, the per-point fingerprint that makes staleness a property of the data, and the three states a visitor can see.
7. **[`05b-projection-rollout.md`](05b-projection-rollout.md)** — the operator's page for the above: what changed since the first draft (Last-300 unified into one module, retired players dropped from the timeline, INITIAL added, full depth, three more fingerprint inputs, triggers on `matches`/`manual_overrides`/`leagues`, and incremental reuse), the deploy order, the mandatory one-time backfill, and a five-part plan for verifying on the live site that the feature costs nothing it should not.
