...
3. **גודל פונט של הטבלה** נבחר ע"י המשתמש בלבד באמצעות אחת משתי מחלקות:
   ...
   בסיס `rem` של הפרויקט (על `<html>`) מוגדר באחוזים כדי לכבד את ברירת המחדל של הדפדפן (נגישות):
   - DESKTOP: `html { font-size: 93.75% }` (≡ 15px אצל משתמש רגיל).
   - MOBILE (≤768px): `html { font-size: 81.25% }` (≡ 13px).

   הבסיס הוא מקור יחיד לאמת, מוגדר בקובץ `layout.css` בלבד.
