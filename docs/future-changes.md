...
- **הוספת ליגה חדשה:** טופס עם שם ליגה + קונפיגורציה (סוג ליגה) → יוצר תיקייה חדשה ב-`leagues/`, מעדכן `leagues_order.json`, יוצר `league_params.json`
- **מחיקת ליגה:** בחירה מרשימה → מוחק תיקייה מ-repo, מעדכן `leagues_order.json`
