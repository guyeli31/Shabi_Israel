# Plan: Separate `scripts/` (External Source sync) into a private repo
...
`scripts/sync-source.js`, that logs into External Source with Playwright + stealth plugin and
...
1. The anti-bot disguise logic (human-like typing, random viewports, fake side-tasks,
   External Source internal selectors) is readable by anyone — including the target site,
...
(`getBaselinePlayedCount` in `scripts/sync-source.js:253-270`) that reads
`leagues/<folder>/leaguedata.csv` + `manual_overrides.json` to compute a post-overrides
baseline and retry export (3 attempts) if External Source returns a truncated export. This
...
`SOURCE_USER`, `SOURCE_PASS` → define them in the new private repo's
Settings → Secrets (referenced at `.github/workflows/sync-source.yml:58-59`).
...
- uses: actions/checkout@v4                 # #2 public repo (the data)
  with:
    ...
    path: public-data
- run: node sync-source.js
  env:
    SOURCE_USER: ${{ secrets.SOURCE_USER }}
    SOURCE_PASS: ${{ secrets.SOURCE_PASS }}
...
Currently `repoRoot = resolve(dirname(...), '..')` (sync-source.js:550) assumes the
...
`try/catch` returning `null` (sync-source.js:267-268). If checkout #2 is missing, the
...
Delete `scripts/sync-source.js`, `scripts/fetch-flag.py`, `scripts/package.json`, and
`.github/workflows/sync-source.yml` from `guyeli31/Shabi_Israel`.
...
- **Git history:** the old sync code stays reachable in the public repo's commit history.
  ...
  guarding against a *targeted* adversary (e.g. External Source operator). Not included here.
- **Phase 2 (Supabase):** once the baseline is read from the DB
  (`getBaselinePlayedCount` Phase-2 branch, sync-source.js:254-256), checkout #2 and the
