...
**Status:** in progress — Phase 1 ✅ Phase 2 ✅ Phase 3 ✅ Phase 4.1 ✅ (shim deleted 2026-06-03) Phase 4.2 ⏳ (B1–B7c + C1–C3 still hand-built) Phase 5 ✅ Phase 6 ✅ Phase 7 ◐ (7.1 wiring landed 2026-06-04 alongside Path-X canonization sweep; 7.2 JS rewiring + 7.3 grep gate + 7.4 destructive deletion still ⏳) Phase 8 ◐ (FF chrome restored in `admin.css` + production wears FF classes via JS; **F5 rewired to `mountFFTable` 2026-08-01** — first production call site; lab tabs / presets / remaining `mountFFTable` wiring not yet done) Phase 9 ⏳
**Owner:** ravivb7
**Created:** 2026-05-11
...
- `td.player-cell` **and `td.league-cell`** (shared rule group) + their `a` styles (color, hover, no-underline). `league-cell` is the clickable League column in C1/C2 — it reuses the OPPONENT (`player-cell`) look so every clickable link cell on the player-general tables is visually identical. Per the link-cell weight contract every link cell in every format renders at font-weight 600 (C1/C2 carry it on the td; other tables on the link class). See docs/TABLE-DESIGN.md §"Link-cell weight contract (ALL formats)".
...
> **F5 (CSV Import Preview) — the first production `mountFFTable` call site.** F5 was MF while the import preview was read-only. Once it grew per-row actions (TA/TB/TD/NP/Undo over the "N updates"), MF stopped fitting — MF has no Action-cell concept — and `js/admin/excelImporter.js` was rewired to **`mountFFTable`** directly, making F5 the one production table already driven by the lab's FF mount while F1–F4/F6/F7 still hand-build the chrome. It is `font-large` like every other FF table (it gave up MF's `font-small` rather than fork the chrome). Per-row state tints live in `css/admin.css` scoped `[data-mf-table-id="F5"]`, to be folded into `ff.css` with the rest at 8.1's final step.
>
> **Two loose ends this created:**
> 1. `buildF5` in `lab-loader.js` + F5's tab in `table-lab/index.html` still build the **old MF shape** — the lab now disagrees with production. Re-point them at an FF preset when 8.1 adds the FF lab tabs (there is no FF tab to move F5 into yet, which is why this was left alone).
> 2. `admin.html` still loads `table-lab/formats/mf/mf.css` even though no MF table remains there — **not dead weight**: it `@import`s `base.css`, which supplies `table.font-large`'s font-size (every FF table is sized by it) and the canonical `.flag` rule, and `admin.html` does not load `css/components.css`. If FF ever gets its own stylesheet link on `admin.html`, `ff.css` `@import`s `base.css` too and this link can then go.
