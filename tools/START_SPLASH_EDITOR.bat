...
REM ============================================================
REM  Splash Editor launcher
REM  Opens splash-poc.html (the loading-screen designer).
REM  The page loads ES modules + CSS via relative paths and reads
REM  assets/splash/splash-config.json, so it MUST be served over
...
if not errorlevel 1 (
    ...
    start "" "http://localhost:8090/shabi-israel/splash-poc.html"
    echo Splash Editor opened. You can close this window.
    timeout /t 2 >nul
    exit /b 0
)
...
timeout /t 2 >nul
start "" "http://localhost:!PORT!/shabi-israel/splash-poc.html"

echo.
echo Splash Editor: http://localhost:!PORT!/shabi-israel/splash-poc.html
