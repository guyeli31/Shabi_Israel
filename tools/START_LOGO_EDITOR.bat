...
REM ============================================================
REM  Logo Editor launcher
REM  Opens logo-editor.html (the ring-text logo designer).
REM  The page loads ES modules + CSS via relative paths and reads
REM  assets/logo/logo.svg, so it MUST be served over http:// (not
REM  opened as a file). "Save to site" rasterizes the logo to
REM  assets/logo/logo.png (640x640) and rewrites logo.svg directly
REM  via the File System Access directory picker (Chrome/Edge).
...
if not errorlevel 1 (
    ...
    start "" "http://localhost:8090/shabi-israel/logo-editor.html"
    echo Logo Editor opened. You can close this window.
    timeout /t 2 >nul
    exit /b 0
)
...
timeout /t 2 >nul
start "" "http://localhost:!PORT!/shabi-israel/logo-editor.html"

echo.
echo Logo Editor: http://localhost:!PORT!/shabi-israel/logo-editor.html
